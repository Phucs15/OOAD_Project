...
public class ProductManagementFrame extends JFrame {
    ...
    private final UndoRedoFrame undoRedoFrame;
    ...
    private void viewWarehouseForSelectedProduct() {
        ...
        String query = """
            SELECT w.Warehouse_Name, w.Latitude, w.Longitude, p.Product_Name, SUM(s.Quantity) AS Total_Quantity
            FROM Store s
            INNER JOIN Warehouse w ON s.Warehouse_ID = w.Warehouse_ID
            INNER JOIN Product p ON s.Product_ID = p.Product_ID
            WHERE p.Product_ID = ? 
            GROUP BY w.Warehouse_Name,w.Latitude, w.Longitude, p.Product_Name;
        """;
        showDataDialog("Warehouse Details", query, new String[]{"Warehouse_Name", "Latitude", "Longitude", "Product_Name", "Total_Quantity"}, productId);
    }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(ProductManagementFrame::new);
    }
}
